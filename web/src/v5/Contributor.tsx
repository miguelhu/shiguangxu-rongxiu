...
import { ArrowRight, Check, Camera, Plus, BookOpen, Sparkle, Trash } from '@phosphor-icons/react'
...
export function Contributor({ page, go }: { page: Page; go: (p: Page) => void }) {
  ...
  const [voiceOpen, setVoiceOpen] = useState(!!d.audio)
  ...
  if (page === 'identity')
    return (
      <div className="phone-body">
        ...
        <Field
          label="联系电话"
          hint="用于后续联系与找回，让这份关系以后还能继续联系。不会展示在礼物、相框或其他共创者页面中。"
        >
          <input
            type="tel"
            value={d.phone}
        ...
        <small className="tiny">本次体验可不填写，不发送验证码。</small>
    ...
    <>
      ...
      <div className="phone-body contributor-body">
        ...
        {page === 'impressions' && (
          <>
              ...
              {[...new Set([...data.authors.flatMap((a) => a.impressions), ...d.tags])]
                ...
                .map((t) => (
                  <button
                    ...
                    key={t}
                    onClick={() => {
                      if (!d.tags.includes(t) && d.tags.length >= 8) {
                        notify('选最有感触的8个就好')
                        return
                      }
                      change({
            ...
            <button
              className="text-button"
              onClick={() => {
                if (customTag.trim() && d.tags.length < 8)
        ...
        {page === 'photos' && (
          <>
            <p className="helper">
              优先选1—3张重要照片，最多10张。给每张照片留一两句话，100字以内。
            </p>
            {d.photos.length > 3 && (
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        update({
                          ...
                          date: e.target.value === 'unknown' ? '' : p.date,
                        })
                      }
                    >
            ...
            {d.photos.length < 10 ? (
              <Upload onPhoto={addPhoto} maxFiles={10 - d.photos.length}>
                <Camera />
                <span>
                  上传照片<small>{d.photos.length}/10 张 · 推荐精选1—3张</small>
                </span>
                <Plus />
              </Upload>
            ) : (
        ...
        {page === 'wishes' && (
          <>
            ...
            >
              送一段语音祝福 <span>{d.audio ? '已加入声音' : '可选'}</span>
            </button>
            {voiceOpen && (
              <VoiceInput
                path={d.audio}
