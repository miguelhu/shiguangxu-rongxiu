...
export default function Frame({
  ...
  const shots = useMemo<Shot[]>(() => {
    ...
    const heroes = (!s.previewing && s.version?.heroes) || s.heroPhotos
    const expected = original.flatMap((s) => s.photoIds)
    if (
      !heroes.length &&
      expected.length === photoList.length &&
      expected.every((id, i) => id === photoList[i].id)
    )
      return original
    const out: Shot[] = []
    let n = 0
    while (n < photoList.length) {
      const size = Math.min(
        heroes.includes(photoList[n].id) || n % 3 === 0 || heroes.includes(photoList[n + 1]?.id)
          ? 1
          : 2,
        photoList.length - n,
      )
    ...
    return out
  }, [c.id, photoList.map((p) => p.id + ':' + p.date).join('|'), s.heroPhotos.join('|')])
  ...
  function move(delta: number) {
    ...
    } else selectChapter((chapter + delta + 7) % 7)
  }
  useEffect(() => {
    if (!playing) return
    const duration =
