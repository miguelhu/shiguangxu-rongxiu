import { Welcome, Activate, SendChoose } from './LifecyclePages'
...
function route() {
  ...
  return {
    ...
    stage: Math.max(
      ...
      Math.min(
        lifecycle.length - 1,
        q.has('stage') ? Number(q.get('stage')) || 0 : stageOf((q.get('page') || 'invite') as Page),
      ),
    ),
  }
}
export default function App() {
  ...
  const [viewStage, setViewStage] = useState(() => {
    const r = route(),
      current = state.cases[state.caseId].page
    return new URLSearchParams(location.search).has('stage') &&
      lifecycle[r.stage]?.pages[state.role] === current
      ? r.stage
      : stageOf(current)
  })
    ...
    setState((old) => ({
      ...
      cases: {
        ...
        [old.caseId]: {
          ...
          page: p,
          ...(['identity', 'impressions', 'photos', 'stories', 'wishes', 'preview'].includes(p)
            ? { lastContributionPage: p, contributionStarted: true }
            : {}),
        },
      },
    }))
  ...
  function phoneContent(p: Page, r: Role = role, navigate: (p: Page) => void = go): ReactNode {
    ...
    if (p === 'invite' || p === 'welcome') return <Welcome go={navigate} />
    if (p === 'product' && r === 'contributor')
      return (
        <div className="phone-body">
          <div className="eyebrow">大家共同完成的礼物</div>
          <h2>看看心意在相框里的样子。</h2>
          <p className="helper">这是内容预览，仪式版本由统筹者确认。</p>
          <Frame compact />
        </div>
      )
    ...
    if (p === 'messages' && r === 'coordinator')
      return (
        <div className="phone-body">
          ...
          <button className="entry-card" onClick={() => navigate('product')}>
            <span>
              <h3>
                {s.version
                  ? '成品已确认'
  ...
  const frameAllowed =
    role === 'coordinator' || (role === 'recipient' && (!!s.version || page === 'activate'))
  const framePage = ['ceremony', 'received', 'activate'].includes(page)
  const blocked = role === 'recipient' && page !== 'activate' && (!s.version || !framePage)
  const reset = () => {
    stopMedia()
    setDialog(null)
    setState((old) => ({
      ...
      <div className={`app mode-${mode} app-v5`}>
        <header className="site-header">
          ...
          <div className="header-actions">
            ...
            <div
              ...
              style={page === 'activate' ? { visibility: 'hidden' } : undefined}
            >
        ...
        <main className="main-stage">
          <div className="role-strip">
            ...
            <small>DEMO 07 · 09·15更新</small>
          </div>
          <div className="breadcrumb">
            <span>{c.category}</span>
            <span>›</span>
            <span>{page === 'activate' ? '拾光叙相框' : s.host.address}</span>
            <span>›</span>
          ...
          {mode === 'overview' ? (
            ...
            <>
              ...
              {blocked ? (
                ...
                frameAllowed ? (
                  role === 'recipient' && (!s.openedAt || page === 'activate') ? (
                    <Activate go={go} />
                  ) : (
                    <>
        ...
        {notes && (
          <aside className="notes-panel">
            ...
            <h2>一份礼物，三个视角。</h2>
            <p>
              固定14个阶段。灰色节点保留但不可点击；角色切换不会替用户提交内容，也不会自动定稿。
            </p>
            ...
            <p>
              小叙使用本地示例整理，未调用云端模型。录音和上传保存在本机；扫码、通知、设备接收为演示。
            </p>
            <Button
              secondary
              onClick={() =>
