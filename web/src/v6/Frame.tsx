...
import { available, ceremonyBlocks, orderedPhotos, orderedStories, photosFor, Block } from './model'
...
export default function Frame({
  ...
  const { c, s, patch, modal, notify } = useDemo()
  ...
  const ids = (!s.previewing && s.version?.storyOrder) || s.storyOrder || stories.map((b) => b.id)
  const featured = [...ids, ...stories.map((b) => b.id).filter((id) => !ids.includes(id))]
    .map((id) => stories.find((b) => b.id === id))
  ...
  const shots = useMemo<Shot[]>(() => {
    ...
    while (n < photoList.length) {
      const first = photoList[n]
      let size = 1
      while (
        size < 4 &&
        n + size < photoList.length &&
        first.date &&
        photoList[n + size].authorId === first.authorId &&
        photoList[n + size].date.slice(0, 7) === first.date.slice(0, 7)
      )
        size++
      out.push({
        id: 'time-' + n,
        layout: size === 1 ? 'hero' : size === 2 ? 'diptych' : 'collage',
    ...
    return out
  }, [photoList.map((p) => p.id + ':' + p.date + ':' + p.caption).join('|')])
  ...
  function move(delta: number) {
    ...
    } else selectChapter((chapter + delta + chapters.length) % chapters.length)
  }
  useEffect(() => {
    if (!playing) return
    const duration =
          ...
          <div className="frame-brand">
            ...
            <button
              ...
              onClick={() => {
                ...
                if (action)
                  action.catch(() => notify('当前浏览器未能进入全屏，请继续在相框中浏览。'))
                else notify('当前浏览器暂不支持全屏，请继续在相框中浏览。')
              }}
            >
              <ArrowsOut size={18} />
            </button>
              ...
              <div className="received-list">
                {s.delivery === 'waiting' ? (
                  ...
                  newItems.map((b) => (
                    ...
                    >
                      {b.photoIds[0] && photosFor(c.id, s).find((p) => p.id === b.photoIds[0]) ? (
                        <PhotoImage
                          path={photosFor(c.id, s).find((p) => p.id === b.photoIds[0])!.path}
                          alt="新送来的照片"
                          className="received-photo"
                        />
                      ) : (
            ...
            <>
              {chapter === 0 && (
                <div className="ceremony-cover">
                  ...
                  <div className="cover-copy">
                    ...
                    <p>{c.subtitle}</p>
                    {c.id === 'birthday' && host.showAge && host.age && (
                      <p>{host.age}岁 · 新的喜欢，慢慢开始</p>
                    )}
              ...
              {chapter === 4 &&
                (() => {
                  ...
                  const blocked = letter.sources.some(
                    (id) =>
                      s.withdrawn.includes(id) ||
                      !available(c.id, s, 'ceremony').some((b) => b.id === id),
                  )
                  return (
                    <div className="letter-frame">
                      <div className="eyebrow">THE WORDS WE SHARE</div>
                      <h2>大家写给你的一封信</h2>
                      {blocked ? (
                        <p>部分来源已撤回，这封信正在等待重新确认。</p>
                      ) : letter.text ? (
                        <>
